...
def winTitle():
    return "原位可视化系统 v{0}".format(currentVersion().__str__())
...
class PyFlow(QMainWindow):
    ...
    def populateMenu(self):
        ...
        newFileAction = fileMenu.addAction("新建文件")
        newFileAction.setIcon(QtGui.QIcon(":/new_file_icon.png"))
        newFileAction.triggered.connect(self.newFile)

        loadAction = fileMenu.addAction("加载")
        ...
        IOMenu = fileMenu.addMenu("自定义 IO")
        for packageName, package in GET_PACKAGES().items():
            ...
            for exporterName, exporterClass in exporters.items():
                ...
                if exporterClass.createExporterMenu():
                    exportAction = fileFormatMenu.addAction("导出")
                ...
                if exporterClass.createImporterMenu():
                    importAction = fileFormatMenu.addAction("导入")
        ...
        preferencesAction = editMenu.addAction("偏好")
        preferencesAction.setIcon(QtGui.QIcon(":/options_icon.png"))
        preferencesAction.triggered.connect(self.showPreferencesWindow)

        pluginsMenu = self.menuBar.addMenu("插件")
        packagePlugin = pluginsMenu.addAction("创建 package...")
        packagePlugin.triggered.connect(PackageWizard.run)

        helpMenu = self.menuBar.addMenu("帮助")
        helpMenu.addAction("主页").triggered.connect(lambda _=False, url="https://wonderworks-software.github.io/PyFlow/": QtGui.QDesktopServices.openUrl(url))
        helpMenu.addAction("文档").triggered.connect(lambda _=False, url="https://pyflow.readthedocs.io/en/latest/": QtGui.QDesktopServices.openUrl(url))
    ...
    def instance(parent=None, software=""):
        ...
        for packageName, registeredToolSet in GET_TOOLS().items():
            for ToolClass in registeredToolSet:
                ...
                if issubclass(ToolClass, DockTool):
                    ...
                    pluginsMenuAction = [m for m in menus if m.title() == "插件"][0].menuAction()
                    toolsMenu = getOrCreateMenu(instance.menuBar, "工具")
        ...
        EditorHistory().saveState("新建 file")
